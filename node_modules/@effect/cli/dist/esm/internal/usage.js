...
/** @internal */
export const named = (names, acceptedValues) => ({
  _tag: "Named",
  names,
  acceptedValues
});
...
/** @internal */
export const repeated = self => ({
  _tag: "Repeated",
  usage: self
});
export const alternation = /*#__PURE__*/dual(2, (self, that) => ({
  _tag: "Alternation",
  left: self,
  right: that
}));
...
export const getHelp = self => {
  ...
  if (Arr.isNonEmptyReadonlyArray(spans)) {
    ...
    if (Arr.isNonEmptyReadonlyArray(tail)) {
      return pipe(Arr.map(spans, span => InternalHelpDoc.p(span)), Arr.reduceRight(InternalHelpDoc.empty, (left, right) => InternalHelpDoc.sequence(left, right)));
    }
...
/** @internal */
export const enumerate = /*#__PURE__*/dual(2, (self, config) => render(simplify(self, config), config));
// =============================================================================
// Internals
// =============================================================================
    ...
    case "Named":
      {
        ...
        const namesToShow = config.showAllNames ? self.names : self.names.length > 1 ? pipe(Arr.filter(self.names, name => name.startsWith("--")), Arr.head, Option.map(Arr.of), Option.getOrElse(() => self.names)) : self.names;
