  ...
  return MessageStorage.of({
    ...
    unregisterReplyHandler: requestId => Effect.sync(() => {
      ...
      for (let i = 0; i < handlers.length; i++) {
        ...
        handler.resume(Effect.fail(new _ClusterError.EntityNotAssignedToRunner({
...
const makeEncoded = exports.makeEncoded = /*#__PURE__*/Effect.fnUntraced(function* (encoded) {
  ...
  const storage = yield* make({
    ...
    repliesForUnfiltered: ids => {
      ...
      return Arr.isNonEmptyArray(requestIds) ? encoded.repliesForUnfiltered(requestIds) : Effect.succeed([]);
    },
    ...
    resetShards: shardIds => {
      const shards = Array.from(shardIds, id => id.toString());
      return Arr.isNonEmptyArray(shards) ? encoded.resetShards(shards) : Effect.void;
    }
  });
