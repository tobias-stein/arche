...
const make = exports.make = /*#__PURE__*/Effect.fnUntraced(function* (options) {
    ...
    switch (envelope._tag) {
      case "Request":
        return {
          ...
          message_id,
          shard_id: _ShardId.ShardId.toString(envelope.address.shardId),
      ...
      case 0:
        return {
          envelope: {
            ...
            address: {
              shardId: _ShardId.ShardId.fromStringEncoded(row.shard_id),
  ...
  return yield* MessageStorage.makeEncoded({
    ...
    }).pipe(Effect.provideService(SqlClient.SafeIntegers, true), _ClusterError.PersistenceError.refail, withTracerDisabled),
    ...
    }).pipe(Effect.asVoid, _ClusterError.PersistenceError.refail, withTracerDisabled),
    ...
    requestIdForPrimaryKey: primaryKey => sql`SELECT id FROM ${messagesTableSql} WHERE message_id = ${primaryKey}`.pipe(Effect.map(rows => Option.fromNullable(rows[0]?.id).pipe(Option.map(Snowflake.Snowflake))), Effect.provideService(SqlClient.SafeIntegers, true), _ClusterError.PersistenceError.refail, withTracerDisabled),
    ...
    sql`
        ...
        ORDER BY rowid ASC
      `.unprepared.pipe(Effect.provideService(SqlClient.SafeIntegers, true), Effect.map(Arr.map(replyFromRow)), _ClusterError.PersistenceError.refail, withTracerDisabled),
    ...
    unprocessedMessages: Effect.fnUntraced(function* (shardIds, now) {
      ...
      return messages;
    }, Effect.provideService(SqlClient.SafeIntegers, true), _ClusterError.PersistenceError.refail, withTracerDisabled),
    unprocessedMessagesById(ids, now) {
      ...
      return sql`
        ...
        AND (m.deliver_at IS NULL OR m.deliver_at <= ${sql.literal(String(now))})
        ORDER BY m.rowid ASC
      `.unprepared.pipe(Effect.map(Arr.map(messageFromRow)), Effect.provideService(SqlClient.SafeIntegers, true), _ClusterError.PersistenceError.refail, withTracerDisabled);
    },
    ...
    clearAddress: address => sql`
      ...
      `.pipe(Effect.andThen(sql`
            ...
            WHERE entity_type = ${address.entityType}
            AND entity_id = ${address.entityId}
          `), sql.withTransaction, Effect.asVoid, _ClusterError.PersistenceError.refail, withTracerDisabled),
