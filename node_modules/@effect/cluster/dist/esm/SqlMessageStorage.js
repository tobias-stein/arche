...
export const make = /*#__PURE__*/Effect.fnUntraced(function* (options) {
  ...
  return yield* MessageStorage.makeEncoded({
    ...
    requestIdForPrimaryKey: primaryKey => sql`SELECT id FROM ${messagesTableSql} WHERE message_id = ${primaryKey}`.pipe(Effect.map(rows => Option.fromNullable(rows[0]?.id).pipe(Option.map(Snowflake.Snowflake))), Effect.provideService(SqlClient.SafeIntegers, true), PersistenceError.refail, withTracerDisabled),
