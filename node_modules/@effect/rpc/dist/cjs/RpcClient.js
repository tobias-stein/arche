...
var _RpcClientError = require("./RpcClientError.js");
...
const makeNoSerialization = exports.makeNoSerialization = /*#__PURE__*/Effect.fnUntraced(function* (group, options) {
  ...
  if (options.flatten) {
    ...
    for (const rpc of group.requests.values()) {
      ...
      const target = prefix !== undefined ? client[prefix] : client;
...
const makeProtocolHttp = client => Protocol.make(Effect.fnUntraced(function* (writeResponse) {
  ...
  const send = request => {
    ...
    if (isJson) {
      return client.post("", {
        body
      }).pipe(Effect.flatMap(r => r.json), Effect.mapError(cause => new _RpcClientError.RpcClientError({
        reason: "Protocol",
    ...
    })), Effect.mapError(cause => new _RpcClientError.RpcClientError({
      reason: "Protocol",
      message: "Failed to send HTTP request",
      cause
    })));
...
const makeProtocolSocket = options => Protocol.make(Effect.fnUntraced(function* (writeResponse) {
  ...
  const pinger = yield* makePinger(write(parser.encode(_RpcMessage.constPing)));
  let currentError;
  ...
  yield* Effect.suspend(() => {
    ...
    return socket.runRaw(message => {
      try {
        ...
        return writeResponse({
          ...
          error: new _RpcClientError.RpcClientError({
            reason: "Protocol",
    ...
    currentError = new _RpcClientError.RpcClientError({
      reason: "Protocol",
  ...
  }), Effect.retry(options?.retrySchedule ?? defaultRetrySchedule), Effect.annotateLogs({
    module: "RpcClient",
...
const defaultRetrySchedule = /*#__PURE__*/Schedule.exponential(500, 1.5).pipe(/*#__PURE__*/Schedule.union(/*#__PURE__*/Schedule.spaced(5000)));
