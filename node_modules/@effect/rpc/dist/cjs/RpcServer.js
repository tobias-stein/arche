...
const make = exports.make = /*#__PURE__*/Effect.fnUntraced(function* (group, options) {
        ...
        case "Chunk":
          {
            ...
            return handleEncode(client, response.requestId, schemas.encodeDefect, schemas.collector, Effect.provide(schemas.encodeChunk(response.values), schemas.context), values => ({
              _tag: "Chunk",
        ...
        case "Exit":
          {
            ...
            return handleEncode(client, response.requestId, schemas.encodeDefect, schemas.collector, Effect.provide(schemas.encodeExit(response.exit), schemas.context), exit => ({
              _tag: "Exit",
              requestId: String(response.requestId),
              exit
            }));
  ...
  const getSchemas = rpc => {
    ...
    if (!schemas) {
      ...
      schemas = {
        ...
        encodeDefect: Schema.encodeUnknown(rpc.defectSchema),
  ...
  const clients = new Map();
  const handleEncode = (client, requestId, encodeDefect, collector, effect, onSuccess) => (collector ? Effect.provideService(effect, Transferable.Collector, collector) : effect).pipe(Effect.flatMap(a => send(client.id, onSuccess(a), collector && collector.unsafeClear())), Effect.catchAllCause(cause => {
    client.schemas.delete(requestId);
    const defect = Cause.squash(Cause.map(cause, _ParseResult.TreeFormatter.formatErrorSync));
    return Effect.zipRight(sendRequestDefect(client, requestId, encodeDefect, defect), server.write(client.id, {
      _tag: "Interrupt",
      requestId,
      interruptors: []
    }));
  ...
  const sendRequestDefect = (client, requestId, encodeDefect, defect) => Effect.catchAllCause(encodeDefect(defect).pipe(Effect.flatMap(encodedDefect => send(client.id, {
    _tag: "Exit",
      ...
      case "Request":
        {
          ...
          return Effect.matchEffect(Effect.provide(schemas.decode(request.payload), schemas.context), {
            onFailure: error => sendRequestDefect(client, requestId, schemas.encodeDefect, _ParseResult.TreeFormatter.formatErrorSync(error)),
            onSuccess: payload => {
